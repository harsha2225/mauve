...
def compute_mauve(
        ...
        verbose=False, seed=25, batch_size=1, use_float64=False, pointwise_mauve=False, distance_metric='l2'
):
    ...
    * ``out.num_buckets`` is the quantization size and the shape of ``out.p_hist`` and ``out.q_hist``.
    ...
    if pointwise_mauve:
        ...
        t1 = time.time()
        q_labels, q_distances = discretize_for_mauve(
            ...
            max_iter=kmeans_max_iter, seed=seed, verbose=verbose, distance_metric=distance_metric, p_texts=p_text, q_texts=q_text
        )
        t2 = time.time()
        ...
        to_return = SimpleNamespace(
            q_labels=q_labels, q_distances=q_distances,
...
def cluster_feats(p, q, num_clusters,
    ...
    t1 = time.time()
    kmeans, _, _, _ = fit_cluster_model(
        data1, num_clusters,
        whiten=whiten,
...
def discretize_for_mauve(p, q, num_clusters,
                  norm='none', whiten=True,
                  pca_max_data=-1,
                  explained_variance=0.9,
                  num_redo=5, max_iter=500,
                  seed=0, verbose=False, distance_metric='l2', 
                  p_texts=None, q_texts=None):
    
    if verbose:
        print(f"\n{'='*80}")
        print(f"DISCRETIZATION DEBUG")
        print(f"{'='*80}")
        print(f"Input P shape: {p.shape}, Q shape: {q.shape}")
    ...
    if verbose:
        print(f"\nPCA: {idx+1} components (explained variance: {np.sum(pca.explained_variance_ratio_[:idx+1]):.4f})")
    ...
    if verbose:
        ...
        print(f"P - shape: {p_transformed.shape}, mean: {np.mean(p_transformed):.6f}, std: {np.std(p_transformed):.6f}")
        print(f"Q - shape: {q_transformed.shape}, mean: {np.mean(q_transformed):.6f}, std: {np.std(q_transformed):.6f}")
    ...
    if verbose:
        print(f"\nCluster centroids shape: {kmeans.centroids.shape}")
        print(f"Centroids - mean: {np.mean(kmeans.centroids):.6f}, std: {np.std(kmeans.centroids):.6f}")
        
        print(f"\n{'='*80}")
        print(f"P (Reference) - ALL SAMPLES: Text | Embedding (used in kmeans) | Distance")
        print(f"{'='*80}")
        print(f"Stats: Mean: {np.mean(p_distances):.6f}, Std: {np.std(p_distances):.6f}, Min: {np.min(p_distances):.6f}, Max: {np.max(p_distances):.6f}, Median: {np.median(p_distances):.6f}")
        print()
        
        for i in range(len(p_distances)):
            text = p_texts[i] if p_texts is not None else f"Sample {i}"
            embedding_str = np.array2string(p_transformed[i], precision=4, separator=', ', suppress_small=True, max_line_width=200)
            embedding_str_data1 = np.array2string(data1[i], precision=4, separator=', ', suppress_small=True, max_line_width=200)
            print(f"[{i}]")
            print(f"  Text: {text}")
            print(f"  Embedding: {embedding_str}")
            print(f"  Embedding: {embedding_str_data1} (original dim)")
            print(f"  Distance: {p_distances[i]:.6f}, Cluster: {p_labels[i]}")
            print()
        
        print(f"\n{'='*80}")
        print(f"Q - ALL SAMPLES: Text | Embedding (used in kmeans) | Distance")
        print(f"{'='*80}")
        print(f"Stats: Mean: {np.mean(q_distances):.6f}, Std: {np.std(q_distances):.6f}, Min: {np.min(q_distances):.6f}, Max: {np.max(q_distances):.6f}, Median: {np.median(q_distances):.6f}")
        print()
        
        for i in range(len(q_distances)):
            text = q_texts[i] if q_texts is not None else f"Sample {i}"
            embedding_str = np.array2string(q_transformed[i], precision=4, separator=', ', suppress_small=True, max_line_width=200)
            print(f"[{i}]")
            print(f"  Text: {text}")
            print(f"  Embedding: {embedding_str}")
            print(f"  Distance: {q_distances[i]:.6f}, Cluster: {q_labels[i]}")
            print()

    centers = kmeans.centroids
    c = centers[q_labels]
    l1_dist = np.sum(np.abs(q_transformed - c), axis=1)
    ...
    cos_dist = cosine_dist(q_transformed, c)

    var = np.var(p_transformed, axis=0) + 1e-6
    maha = np.sum(((q_transformed - c) ** 2) / var, axis=1)

    l2 = np.linalg.norm(q_transformed - c, axis=1)
    cos = 1 - np.sum(q_transformed * c, axis=1)
    ...
    t2 = time.time()
    if verbose:
        print(f'\nKmeans time: {round(t2-t1, 2)} s')
        print(f'\nUsing distance metric: {distance_metric}')
