...
def compute_mauve(
        p_features=None, q_features=None, test_features=None,
        p_tokens=None, q_tokens=None, test_tokens=None,
        p_text=None, q_text=None, test_text=None,
        ...
        verbose=False, seed=25, batch_size=1, use_float64=False, pointwise_mauve=False, distance_metric='l2', **kwargs
):
    ...
    :param ``test_features``: ``numpy.ndarray`` of shape (m, d), where m is the number of test generations.
    ...
    :param ``test_tokens``: list of length m, each entry is torch.LongTensor of shape (1, length).
    :param ``p_text``: list of length n, each entry is a string.
    :param ``q_text``: list of length n, each entry is a string.
    :param ``test_text``: list of length m, each entry is a string.
    ...
    :param ``pointwise_mauve``: If True, compute pointwise mauve scores for each point in P against Q.
    ...
    * ``out.num_buckets`` is the quantization size and the shape of ``out.p_hist`` and ``out.q_hist``.
    ...
    if test_text is not None and len(test_text) !=0:
        test_features = get_features_from_input(
            test_features, test_tokens, test_text, featurize_model_name, max_text_length,
            device_id, name="test", verbose=verbose, batch_size=batch_size, use_float64=use_float64,
        )
    ...
    if pointwise_mauve:
        # Distance calculations for each point in p_features against q_features.
        t1 = time.time()
        test_labels, test_distances, test_kmeans_pca = discretize_for_mauve(
            p_features, q_features, test_features, num_clusters=num_buckets,
            norm='l2', whiten=False, pca_max_data=pca_max_data,
            explained_variance=kmeans_explained_var, num_redo=kmeans_num_redo,
            max_iter=kmeans_max_iter, seed=seed, verbose=verbose, distance_metric=distance_metric, **kwargs
        )
        t2 = time.time()
        ...
        to_return = SimpleNamespace(
            test_labels=test_labels, test_distances=test_distances,
            num_buckets=num_buckets, test_kmeans_pca = test_kmeans_pca,
        )
        return to_return
    else:
...
def fit_cluster_model(data1, num_clusters,
                  whiten=True,
                  pca_max_data=-1,
    ...
    if verbose:
        print("\nCentroid embeddings (label → embedding):")
        for label, centroid in enumerate(kmeans.centroids):
            print(f"Label {label}: {centroid}")


    return kmeans, pca, idx, data1
...
def cluster_feats(p, q, num_clusters,
    ...
    t1 = time.time()
    kmeans, _, _, data1_transformed = fit_cluster_model(
        data1, num_clusters,
        whiten=whiten,
        pca_max_data=pca_max_data,
        explained_variance=explained_variance,
        num_redo=num_redo, max_iter=max_iter,
        seed=seed, verbose=verbose
    )
    _, labels = kmeans.index.search(data1_transformed, 1)
...
def discretize_for_mauve(p, q, test, num_clusters,
                  norm='none', whiten=True,
                  pca_max_data=-1,
                  explained_variance=0.9,
                  num_redo=5, max_iter=500,
                  seed=0, verbose=False, distance_metric='l2', **kwargs):
    
    if verbose:
        print(f"\n{'='*80}")
        print(f"DISCRETIZATION DEBUG")
        print(f"{'='*80}")
        print(f"Input P shape: {p.shape}, Q shape: {q.shape}, Test shape: {test.shape}")
        print(f"P - mean: {np.mean(p):.6f}, std: {np.std(p):.6f}, min: {np.min(p):.6f}, max: {np.max(p):.6f}")
        print(f"Q - mean: {np.mean(q):.6f}, std: {np.std(q):.6f}, min: {np.min(q):.6f}, max: {np.max(q):.6f}")
        print(f"TEST - mean: {np.mean(test):.6f}, std: {np.std(test):.6f}, min: {np.min(test):.6f}, max: {np.max(test):.6f}")
    
    min_size = min(p.shape[0], q.shape[0])
    if p.shape[0] > min_size:
        rng = np.random.RandomState(seed+10)
        idxs = rng.choice(p.shape[0], size=min_size, replace=False)
        p = p[idxs]
        if verbose:
            print(f"Downsampled P to shape: {p.shape}")

    if q.shape[0] > min_size:
        rng = np.random.RandomState(seed+20)
        idxs = rng.choice(q.shape[0], size=min_size, replace=False)
        q = q[idxs]
        if verbose:
            print(f"Downsampled Q to shape: {q.shape}")
    ...
    assert norm in ['none', 'l2', 'l1', None]
    if norm in ['l2', 'l1']:
        p = normalize(p, norm=norm, axis=1)
        q = normalize(q, norm=norm, axis=1)
        if verbose:
            print(f"\nAfter {norm} normalization:")
            print(f"P - mean: {np.mean(p):.6f}, std: {np.std(p):.6f}")
            print(f"Q - mean: {np.mean(q):.6f}, std: {np.std(q):.6f}")
            print(f"TEST - mean: {np.mean(test):.6f}, std: {np.std(test):.6f}")
    ...
    if kwargs.get('kmeans') is not None:
        kmeans = kwargs['kmeans']
        pca = kwargs['pca']
        idx = kwargs['idx']
        if verbose:
            print("Using precomputed kmeans and pca models for discretization.")
    else:
        kmeans, pca, idx, data1 = fit_cluster_model(
            data1, num_clusters,
            whiten=whiten,
            pca_max_data=pca_max_data,
            explained_variance=explained_variance,
    ...
    print("Data shape for clustering:", data1.shape)
    ...
    if verbose:
        ...
        for i in range(num_clusters):
            print(f"Cluster {i}: P count = {p_bin_counts[i]}, Q count = {q_bin_counts[i]}")

    p_transformed = pca.transform(p)[:, :idx+1]
    q_transformed = pca.transform(q)[:, :idx+1]
    test_transformed = pca.transform(test)[:, :idx+1]
    
    p_transformed = p_transformed.astype(np.float32)
    q_transformed = q_transformed.astype(np.float32)
    test_transformed = test_transformed.astype(np.float32)

    p_distances, p_labels = kmeans.index.search(p_transformed, 1)
    p_distances = p_distances.reshape(-1)
    p_labels = p_labels.reshape(-1)
    
    q_distances, q_labels = kmeans.index.search(q_transformed, 1)
    q_labels = q_labels.reshape(-1)
    q_distances = q_distances.reshape(-1)

    test_distances, test_labels = kmeans.index.search(test_transformed, 1)
    test_labels = test_labels.reshape(-1)
    test_distances = test_distances.reshape(-1)
    test_p_counts = p_bin_counts[test_labels]
    test_q_counts = q_bin_counts[test_labels]

    test_weights = (test_q_counts + 0.1) / (test_p_counts + 0.1)
    if verbose:
        print(f"\nTest point cluster assignments and counts:")
        for i in range(len(test_labels)):
            print(f"Test point {i}: Cluster = {test_labels[i]}, P count = {test_p_counts[i]}, Q count = {test_q_counts[i]}, Distance = {test_distances[i]:.6f}, Weight = {test_weights[i]:.6f}")

    centers = kmeans.centroids
    c = centers[test_labels]
    l1_dist = np.sum(np.abs(test_transformed - c), axis=1)
    ...
    cos_dist = cosine_dist(test_transformed, c)

    var = np.var(data1, axis=0) + 1e-6
    maha = np.sum(((test_transformed - c) ** 2) / var, axis=1)

    l2 = np.linalg.norm(test_transformed - c, axis=1)
    cos = 1 - np.sum(test_transformed * c, axis=1)

    alpha, beta = 1.0, 1.0
    score = alpha * l2 + beta * cos
    ...
    if distance_metric == 'l2':
        ...
        test_distances = l1_dist
    elif distance_metric == 'cosine':
        test_distances = cos_dist
    elif distance_metric == 'mahalanobis':
        test_distances = maha
    elif distance_metric == 'combined':
        test_distances = score
    else:
        test_distances = test_distances
     
    return test_labels, test_distances * test_weights, {'kmeans': kmeans, 'pca': pca, 'idx': idx, 'p_features': p_features, 'q_features': q_features, 'test_p_counts': test_p_counts, 'test_q_counts': test_q_counts, 'test_distances_raw': test_distances}
